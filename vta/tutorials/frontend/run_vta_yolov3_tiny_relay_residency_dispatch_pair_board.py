#!/usr/bin/env python3
"""Clean-start board comparison of an exact-dispatch YOLOv3-tiny pair."""
...
RANDOM_SEEDS = (20250901, 20260910)
ROUNDS = 7
...
def load_executor(remote, build_dir, mode, contexts, temporary):
    ...
    remote_name = mode + "_yolov3_tiny.so"
...
def image_input(path):
    data = darknet.load_image(str(path), 416, 416).transpose(1, 2, 0)
    return np.ascontiguousarray(data.transpose(2, 0, 1)[None, :].astype("float32"))


def random_input(seed):
    rng = np.random.default_rng(int(seed))
    return rng.uniform(0.0, 1.0, size=(1, 3, 416, 416)).astype("float32")
...
def invoke(executor, data, functions, device, timed):
    ...
    outputs = [executor.get_output(index).numpy() for index in range(executor.get_num_outputs())]
    return outputs, {
...
def mismatch_count(lhs, rhs):
    if len(lhs) != len(rhs):
        raise RuntimeError("A/B output count differs")
    return [int(np.count_nonzero(a != b)) for a, b in zip(lhs, rhs)]
...
def run(args):
    ...
    allowed_status = {
        "yolov3_tiny_pair_cross_build_dispatch_verified",
        "yolov3_tiny_stock_selected_cross_build_verified",
    }
    if build["status"] not in allowed_status:
        raise ValueError("input build is not verified")
    modes = [row["public_mode"] for row in build["builds"]]
    if modes not in (["original", "input_stationary"], ["stock_tophub", "input_stationary"]):
        raise ValueError("unexpected modes")
    ...
    fresh_rpc = board.start_rpc(args.default_runtime, "p7r235_yolov3_dispatch.log")
    ...
    correctness = []
    timings = []
    with tempfile.TemporaryDirectory(prefix="c3_yolo_dispatch_") as temporary:
        executors = {
            mode: load_executor(remote, build_dir, mode, contexts, temporary) for mode in modes
        }
        cases = [("person_image", image)] + [
            ("random_{}".format(seed), random_input(seed)) for seed in RANDOM_SEEDS
        ]
        for case, data in cases:
            outputs = {}
            for mode in modes:
                actual, row = invoke(executors[mode], data, functions, device, False)
                outputs[mode] = actual
                row.update(case=case, public_mode=mode)
                correctness.append(row)
            mismatches = mismatch_count(outputs[modes[0]], outputs[modes[1]])
            ...
            if any(mismatches):
                ...
                raise RuntimeError("whole-model YOLO A/B output mismatch")
        for round_index in range(ROUNDS):
            order = modes if round_index % 2 == 0 else list(reversed(modes))
            round_outputs = {}
            for position, mode in enumerate(order):
                actual, row = invoke(executors[mode], image, functions, device, True)
                round_outputs[mode] = actual
                row.update(round=round_index, position=position, public_mode=mode)
                timings.append(row)
            mismatches = mismatch_count(round_outputs[modes[0]], round_outputs[modes[1]])
            for row in timings[-2:]:
                row["paired_mismatch_count_by_output"] = mismatches
                row["paired_equal"] = all(value == 0 for value in mismatches)
            if any(mismatches):
                raise RuntimeError("timed whole-model YOLO A/B output mismatch")
    ...
    summary = {
        "schema": "c3_vta_yolov3_tiny_exact_residency_dispatch_board_v1",
        "status": "yolov3_tiny_pair_board_correctness_and_timing_complete",
        ...
        "board_after": after,
        "claim_boundary": "Full YOLO graph paired-output equivalence; no mAP or physical AXI evaluation",
    }
