#!/usr/bin/env python3
"""Run the frozen W04 tile-height correctness boundary probe."""
...
def load_probe(path):
    ...
    if ledger["output_sha256"].get(path.name) != file_sha256(path):
        raise ValueError("height-probe contract hash mismatch")
    probe = json.loads(path.read_text())
    if probe.get("schema") != "c3_w04_height_probe_contract_v1" or probe.get(
        "status"
    ) != "frozen_before_new_probe_labels":
        raise ValueError("unexpected or unfrozen height-probe contract")
    return probe
...
def main():
    ...
    if results_path.exists() or (output / "summary.json").exists() or (output / "failure.json").exists():
        raise FileExistsError("refusing to overwrite height-probe observations")
    ...
    rows = []
    try:
        with tempfile.TemporaryDirectory(prefix="c3_w04_height_") as temporary:
            ...
            with results_path.open("x", encoding="utf-8") as stream:
                for position, candidate_id in enumerate(probe["board_order"], 1):
                    ...
                    row = {
                        "schema": "c3_w04_height_probe_result_v1",
                        "probe_position": position,
                        "candidate_id": candidate_id,
                        "config_index": entry["config_index"],
                        "tile_h": entry["split_knobs"]["tile_h"],
                        "accumulator_vectors_per_residency_region": entry[
                            "accumulator_vectors_per_residency_region"
                        ],
                        "tir_sha256": tir_hash,
                        ...
                        "all_correct": all(seed["correct"] for seed in seed_rows),
                    ...
                    stream.flush()
                    print(
                        "{}/{} config={} tile_h={} acc_vectors={} mismatches={} sentinel=exact".format(
                            position,
                            len(probe["board_order"]),
                            entry["config_index"],
                            entry["split_knobs"]["tile_h"],
                            entry["accumulator_vectors_per_residency_region"],
        ...
        write_json(
            output / "failure.json",
            {
                "schema": "c3_w04_height_probe_failure_v1",
    ...
    inherited_entry = next(row for row in probe["candidates"] if row["config_index"] == 139)
    combined.append(
        {
            "config_index": 139,
            "tile_h": 14,
            "accumulator_vectors": inherited_entry["accumulator_vectors_per_residency_region"],
            "all_correct": False,
            "evidence": "inherited_p7c1_three_repeats",
        }
    )
    combined.sort(key=lambda row: row["tile_h"])
    write_json(
        output / "summary.json",
        {
            "schema": "c3_w04_height_probe_summary_v1",
            "status": "complete",
            "new_candidates": len(rows),
            "combined_height_sweep": combined,
