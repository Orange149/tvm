#!/usr/bin/env python3
"""Audit one prospective R50A call-site latency holdout without rewriting raw data."""
...
def run(args):
    ...
    if args.expected_family_id and contract.get("selected_family_id") != args.expected_family_id:
        raise RuntimeError("unexpected selected family")
    if any(
        contract[key]
        for key in (
    ...
    if len(board["singleton_results"]) != len(manifest["routes"]):
        raise RuntimeError("singleton result count differs from the frozen route count")
    for result, route in zip(board["singleton_results"], manifest["routes"]):
        if not all(result["checks"].values()):
            raise RuntimeError("a singleton failed its correctness/DMA/timing contract")
        if result["expected_delta"] != route["expected_delta"]:
            raise RuntimeError("singleton expected delta differs from fused TIR")
        if result["correctness_profile_delta"] != result["expected_delta"]:
            raise RuntimeError("singleton correctness profile differs from fused TIR")
        if result["timed_profile_delta"] != result["expected_delta"]:
            raise RuntimeError("singleton timing profile differs from fused TIR")
    group = board["group_result"]
    if not all(group["checks"].values()):
        raise RuntimeError("whole-group correctness/DMA/timing contract failed")
    if group["expected_delta"] != fused["full_graph_fused_tir_delta"]:
        raise RuntimeError("whole-group expected delta differs from fused TIR")
    if group["correctness_profile_delta"] != group["expected_delta"]:
        raise RuntimeError("whole-group correctness profile differs from fused TIR")
    if group["timed_profile_delta"] != group["expected_delta"]:
        raise RuntimeError("whole-group timing profile differs from fused TIR")

    clean_starts = sorted(board_dir.glob("clean_start_*.json"))
    if len(clean_starts) != len(manifest["routes"]) + 1:
        raise RuntimeError("expected one clean start for every singleton and whole group")

    output = Path(args.output_dir)
    if output.exists():
        raise FileExistsError("refusing to overwrite " + str(output))
    output.mkdir(parents=True)
    family = contract["selected_family_id"]
    result = {
        "schema": "c3_vta_r50a_callsite_latency_holdout_audit_v1",
        ...
        * 100.0,
        ...
        "clean_start_count": len(clean_starts),
        "auditor_source_sha256": sha256(Path(__file__).resolve()),
        "raw_metadata_correction": (
            "The subset builder and board runner reused stale F00/development wording. "
            "Their immutable measurements are retained; pre-board content hashes and this "
            "audit establish the authoritative family-specific latency-holdout scope"
        ),
        "claim_boundary": (
            "One additional tile on the same R50A workload, model and boot. This is a "
            "call-site/full-graph latency holdout, not a new operator workload, cross-boot "
            "result, ImageNet accuracy, native AutoTVM call-site identity, natural failed "
            "bundle, or physical AXI measurement"
        ),
    }
